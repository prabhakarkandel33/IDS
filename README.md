...
**App won't start — sklearn version warning becomes error**
→ Run: `pip install scikit-learn==1.8.0`

**ModuleNotFoundError on launch**
→ Make sure you ran `conda activate ids` before `python app.py`

**"Missing columns" error on upload**
→ Your CSV was not generated by CICFlowMeter or has been modified.
   The app expects the raw CICFlowMeter output without column changes.
